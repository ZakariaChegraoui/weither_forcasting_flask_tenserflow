...
def get_plots():
    ...
    df = pd.DataFrame(t, columns = ['Date','DIFFUSED RADIATION (W/M^2)','GLOBAL RADIATION (W/M^2)',
    ...
    ax =  sns.lineplot(data=df['TEMPERATURE (°C)'], markers=True,dashes= False)
    ax =  sns.lineplot(data=df['MAXIMUM TEMPERATURE(°C)'], markers=True,dashes= False)
    ax =  sns.lineplot(data=df['MINIMUM TEMPERATURE (°C)'], markers=True,dashes= False)

    output = io.BytesIO()
    FigureCanvas(fig).print_png(output)
    #plt.savefig("static/seaborn_plot.png", transparent=True,dpi=300)
    plt.close()
